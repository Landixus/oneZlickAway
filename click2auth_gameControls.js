...
// ==========================================
// Platzhalter - echte Daten werden aus credentials.ini geladen
let ZWIFT_USER = "YOUR_ZWIFT_LOGIN_MAIL"; 
let ZWIFT_PASS = "YOUR_ZWIFT_PASS";      
let TARGET_SERIAL = "YOUR_DEVICE_SERIAL"; 

let DYNAMIC_MAP = {}; 
const DEBOUNCE_MS = 20; // Zeitfenster für Entprellung in Millisekunden
...
function loadConfig() {
    ...
    if (fs.existsSync(iniPath)) {
        try {
            ...
            console.log(`📁 Konfiguration aus credentials.ini geladen.`);
        } catch (e) { console.error(`⚠️ Fehler beim Laden der INI: ${e.message}`); }
    } else {
        console.log(`ℹ️ Keine credentials.ini gefunden, nutze Standard-Platzhalter.`);
    }
}
loadConfig();

// ==========================================
// ====== POWERSHELL BRIDGE (STABIL) ======
// ==========================================
let ps = null;
function startPS() {
...
'@
    ...
    ps.stdout.on('data', (d) => {
        if(d.toString().includes("PS_READY")) console.log("⌨️ Tastatur-Schnittstelle bereit.");
    });
}
startPS();
...
const sendKeyUpdate = (vk, isDown) => {
    ...
    console.log(`  ${isDown ? '⬇️ [DRUCK]' : '⬆️ [RELEASE]'} Taste: ${name}`);
...
let lastPressed32 = 0;
let lastChangeTimes = new Array(32).fill(0); // Speicher für Entprellung
let zwiftToken = null;
...
async function getZwiftToken() {
    try {
        console.log(`🌐 Login bei Zwift für ${ZWIFT_USER}...`);
        ...
        if (res.data?.access_token) {
            console.log("✅ Zwift Login erfolgreich.");
            return res.data.access_token;
        }
    } catch (e) { console.error("❌ Login-Fehler bei Zwift."); }
    return null;
}

async function proxyAuth(payload, token) {
    const offsets = [5, 3, 4, 2, 0]; 
    for (const o of offsets) {
        try {
            ...
            if (res.status === 200) {
                console.log("✅ Challenge vom Server gelöst.");
...
async function connectClick(peripheral) {
    ...
    try {
        console.log(`🔗 Verbinde mit Click ${peripheral.id}...`);
        ...
        rx.on("data", (data) => {
            ...
            if (first === 0x19 && data[1] === 0x10 && data[2] === 0x50) {
                console.log("🎯 Hardware freigeschaltet! Tasten aktiv.");
            ...
            if (first === 0xff && data[1] === 0x03 && zwiftToken && currentStatus === ST_IDLE) {
                if (data.length === 85) {
                    console.log("🔑 Spezial-Challenge (85b) erkannt.");
                    tx.write(Buffer.from([0xff, 0x04, 0x00]), true);
                } else {
                    console.log("🔑 Challenge erkannt. Berechne Antwort...");
        ...
        await rx.subscribe();
        console.log("👂 Bereit. Warte auf Hardware-Handshake...");

        const nudge = () => {
            process.stdout.write("👉 Rufe Click auf... ");
        ...
        const monitorInterval = setInterval(() => {
            ...
            if (currentStatus === ST_IDLE) {
                ...
                // Keep-Alive Herzschlag (0x47 0x00) alle 10 Sek
...
async function main() {
    ...
    if (!zwiftToken) {
        console.log("❌ Programm beendet: Kein Zwift-Token verfügbar.");
        return;
    }
